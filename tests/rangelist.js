...
]) test(`${JSON.stringify(start)} -> add ${JSON.stringify(add)}`, t => {
  const rl = start.slice(0);
  for (let i = 0; i < add.length;) {
    const begin = add[i++], end = add[i++];
    rangeListAdd(rl, begin, end);
  }
  t.deepEqual(rl, expect);
});
